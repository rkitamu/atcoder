...
func main() {
	...
	a := NewVectorFromPointsSlice(ax, ay)
	b := NewVectorFromPointsSlice(bx, by)

	// bx と by が aを はさむ
	axbx := NewVectorFromPointsSlice(ax, bx)
	axby := NewVectorFromPointsSlice(ax, by)
	axbxCross := a.Cross(*axbx)
	axbyCross := a.Cross(*axby)
	abx := false
	if 0 <= axbxCross && axbyCross <= 0 || 0 >= axbxCross && axbyCross >= 0 {
		abx = true
	}

	// ax と ay が bを はさむ
	bxax := NewVectorFromPointsSlice(bx, ax)
	bxay := NewVectorFromPointsSlice(bx, ay)
	bxaxCross := b.Cross(*bxax)
	bxayCross := b.Cross(*bxay)
	bax := false
	if 0 <= bxaxCross && bxayCross <= 0 || 0 >= bxaxCross && bxayCross >= 0 {
		bax = true
	}
	...
	// 1直線の場合
	if 0 == axbxCross && axbyCross == 0 && 0 == bxaxCross && bxayCross == 0 {
		aStart := NewPair(ax[0], ax[1])
		bStart := NewPair(bx[0], bx[1])
		aEnd := NewPair(ay[0], ay[1])
		bEnd := NewPair(by[0], by[1])
		if bEnd.Lt(bStart) {
			Swap(&bStart, &bEnd)
		}
		if aEnd.Lt(aStart) {
