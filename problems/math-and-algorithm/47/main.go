...
func main() {
	...
	isBi := true
	var dfs func(node *Node, class bool)
	dfs = func(node *Node, class bool) {
		if !isBi {
			return
		}
		...
		if !node.Visited {
			...
			for i := 0; i < len(node.Nexts); i++ {
				dfs(g.GetNode(node.Nexts[i]), !class)
			}
		} else if node.Class != class {
			isBi = false
		}
	}
	...
	for i := 1; i < len(g.Nodes); i++ {
		if !g.GetNode(i).Visited {
			dfs(g.GetNode(i), false)
			if !isBi {
				break
			}
		}
