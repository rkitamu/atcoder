...
func main() {
	...
	var ans int
	ans += cards[0] * (cards[0] - 1) / 2
	ans += cards[1] * (cards[1] - 1) / 2
	ans += cards[2] * (cards[2] - 1) / 2
	fmt.Println(ans)
}
...
func comb(n, k int) int64 {
	...
	return int64(factorial(n) / (factorial(k) * factorial(n-k)))
}

var factTable = make([]int64, 0)
func factorial(n int) int64 {
	...
	for i := len(factTable); i <= n; i++ {
		if i == 0 {
			factTable = append(factTable, 1)
		} else {
			factTable = append(factTable, factTable[i-1]*int64(i))
		}
	}
	return factTable[n]
}

func factorialb(n int) int64 {
	if n < 0 {
		panic("factorial: n must be non-negative")
	}
	res := int64(1)
