...
func main() {
	...
	for i := 0; i <= w; i++ {
		for j := 0; j <= n; j++ {
			...
			w, v := wv[j-1][0], wv[j-1][1]
			if i < w { dp[i][j] = belowValue; continue } // 現在の品物の重さが現在の容量より大きい場合は選べない
			currentValue := dp[i-w][j-1] + int64(v)
			
			dp[i][j] = maxInt64(belowValue, currentValue)
		}
	}
